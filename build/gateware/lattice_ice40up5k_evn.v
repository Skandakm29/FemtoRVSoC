...
//
// Filename   : lattice_ice40up5k_evn.v
// Device     : ice40-up5k-sg48
// LiteX sha1 : eda4e49b7
// Date       : 2025-07-11 18:50:39
//------------------------------------------------------------------------------
...
always @(*) begin
    ...
    slave_sel[2] <= (shared_adr[29:4] == 25'd16777216);
...
assign sram1_adr = interface1_ram_bus_adr[3:0];
...
reg [31:0] rom[0:3551];
initial begin
	$readmemh("lattice_ice40up5k_evn_rom.init", rom);
end
reg [31:0] rom_dat0;
always @(posedge sys_clk) begin
	rom_dat0 <= rom[adr];
...
initial begin
	$readmemh("lattice_ice40up5k_evn_sram.init", sram);
end
reg [10:0] sram_adr0;
always @(posedge sys_clk) begin
	if (sram0_we[0])
...
//------------------------------------------------------------------------------
// Memory main_ram: 16-words x 32-bit
//------------------------------------------------------------------------------
// Port 0 | Read: Sync  | Write: Sync | Mode: Write-First | Write-Granularity: 8 
reg [31:0] main_ram[0:15];
initial begin
	$readmemh("lattice_ice40up5k_evn_main_ram.init", main_ram);
end
reg [3:0] main_ram_adr0;
always @(posedge sys_clk) begin
	if (sram1_we[0])
...
reg [7:0] mem[0:27];
initial begin
	$readmemh("lattice_ice40up5k_evn_mem.init", mem);
end
reg [4:0] mem_adr0;
always @(posedge sys_clk) begin
...
// -----------------------------------------------------------------------------
//  Auto-Generated by LiteX on 2025-07-11 18:50:39.
//------------------------------------------------------------------------------
